...
config["auth_token"] = st.sidebar.text_input(
    "Token", config.get("auth_token", ""), type="password", key="token_input"
).strip()
...
def batch_update_tracks(release_id, track_list, api: MusicAlligatorClient):
    st.write("→ Updating track metadata…")
    for meta in track_list:
        tid = meta.get("trackId")
        data = {k: v for k, v in meta.items() if k != "trackId"}
        ok = api.update_track(release_id, tid, data)
        st.write(f"Track {tid} update: {'OK' if ok else 'FAIL'}")
...
def set_release_label(
    release_id: int, label_id: int, year: int, api: MusicAlligatorClient
):
    ...
    ok = api.update_release(release_id, data)
    st.write(f"Label update: {'OK' if ok else 'FAIL'}")


def set_streaming_platforms(
    release_id: int, platforms: list[int], api: MusicAlligatorClient
):
    data = {"streamingPlatforms": platforms}
    st.write("→ Setting platforms…")
    ok = api.set_platforms(release_id, platforms)
    st.write(f"Platforms update: {'OK' if ok else 'FAIL'}")


def upload_release(base, files, opts):
    local = MusicAlligatorClient(config["auth_token"])
    ...
    draft = local.create_release()
    if not draft:
        st.error("Create failed")
        return
    rid = draft["releaseId"]
    track0 = draft["tracks"][0]["trackId"]
    ...
    ok = local.update_release(rid, meta_release)
    st.write(f"Metadata updated: {'OK' if ok else 'FAIL'}")
    ...
    # 3) Upload cover
    if "cover" in files:
        with st.spinner("→ Uploading cover…"):
            ok = local.upload_cover(rid, files["cover"])
        st.write(f"Cover upload: {'OK' if ok else 'FAIL'}")

    # 4) Upload audio на правильный endpoint
    if "audio" in files:
        with st.spinner("→ Uploading audio…"):
            ok = local.upload_audio(rid, track0, files["audio"])
        st.write(f"Audio upload: {'OK' if ok else 'FAIL'}")
        if not ok:
