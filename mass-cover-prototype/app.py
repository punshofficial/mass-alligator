...
with st.sidebar:
    ...
    config["blend_mode"] = st.selectbox(
        ...
        index=["overlay", "multiply", "screen"].index(
            config.get("blend_mode", "overlay")
        ),
    )
    config["opacity"] = st.slider(
        "Непрозрачность (%)", 0, 100, config.get("opacity", 24)
    )
    ...
    sel = st.selectbox(
        ...
        dirs,
        index=(
            dirs.index(config.get("output_dir"))
            if config.get("output_dir") in dirs
            else 0
        ),
    )
    if sel == "<Создать новую>":
        config["output_dir"] = st.text_input(
            "Имя новой папки", value=config.get("output_dir")
        )
    else:
...
if st.button("▶️ Run"):
    ...
    if config.get("texture_path"):
        tex_img = (
            Image.open(config["texture_path"])
            .convert("RGB")
